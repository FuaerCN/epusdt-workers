...
export class OrderController {
  static async createTransaction(c) {
    ...
    if (!body.order_id || !body.amount || !body.notify_url || !body.signature) {
       return c.json(error(Errno.PARAMS_MARSHAL_ERR, ErrMsg[Errno.PARAMS_MARSHAL_ERR]));
    }

    // 2. Verify Signature
    if (!verifySignature(body, env.API_AUTH_TOKEN, body.signature)) {
      return c.json(error(Errno.SIGNATURE_ERR, ErrMsg[Errno.SIGNATURE_ERR]));
    }
    ...
    try {
      ...
      if (e.code) {
        return c.json(error(e.code, e.message));
      }
      return c.json(error(Errno.SYSTEM_ERROR, e.message || ErrMsg[Errno.SYSTEM_ERROR]));
    }
  }
}
